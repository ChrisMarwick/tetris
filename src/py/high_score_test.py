...
def dummy_storage(reset_global_high_score):
    return InMemoryStorage({'high_score': [
        ('Chris', 1000),
        ('Albert', 900),
        ('Bob', 850),
        ('Dylan', 700),
...
class TestHighScore:
    ...
    @pytest.mark.parametrize('name,new_score,expected_index,expected_high_scores', [
        (
                'Luke', 1100, 0, [
                    ('Luke', 1100),
                    ('Chris', 1000),
    ...
    def test_add_score_new_high_score(self, dummy_storage, name, new_score, expected_index, expected_high_scores):
        high_score = HighScore(dummy_storage)
